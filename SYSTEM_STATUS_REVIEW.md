...
- **Doanh nghiệp ứng dụng**: Hệ sinh thái dịch vụ chuyển đổi số Mắt Bão, phục vụ 2 đơn vị độc lập:
  - **🏢 MBC (Matbao-corp)**: Hạ tầng Tên Miền, Hosting (Vibe Host, Cloud Hosting), Google Workspace, M365 Copilot, Email Doanh nghiệp, Chứng chỉ số (VMC, S-MIME).
  - **🧾 MBI (Matbao-invoice)**: Nền tảng Hóa đơn điện tử, Hợp đồng điện tử, Hóa đơn đầu vào, Chữ ký số (Cloud CA).
...
- **Tổ chức 2 khối thương hiệu (MBC vs MBI)**:
  - Bộ lọc Segmented Control inline (`Tất cả (12)`, `🏢 MBC (8)`, `🧾 MBI (4)`).
...
- **Thẻ Tên Miền thiết kế đặc thù**: Hỗ trợ gắn thẻ danh sách các nhóm đuôi mở rộng (`.VN / .COM`, `.CLOUD`, `.IO`, `.AI`, `.XYZ`, `.ASIA`, `.ICU`...).
...
- **Modal 4 bước định hướng**:
  ...
  - Bước 4: Chọn phân quyền duyệt: Người lập (**Lê Phạm Minh Châu**) và Người duyệt (**Lâm Quang Thịnh**).
...
- **Slide-over Feedback & Kiểm Duyệt (~40% màn hình)**:
  ...
  - **Tính năng Auto-QA kiểm tra Creative**: Tự động rà soát mật độ chữ $(< 20\%)$, mã màu thương hiệu Mắt Bão (`#2563EB` / `#059669`), nút kêu gọi hành động (CTA).
...
- **Nút "✨ Chạy Phân Tích" chủ động**: Sau khi sửa số liệu, người dùng chủ động kích hoạt hệ thống tự động tổng hợp tỷ lệ đạt KPI của MBC & MBI, viết nhận định điều hành và action plan.
...
- **Cơ sở dữ liệu nhân sự thực tế (Source of Truth)**:
  1. `Lâm Quang Thịnh` - Acting Marketing Manager (Người duyệt kế hoạch / Kính gửi báo cáo).
  2. `Lê Phạm Minh Châu` - Senior Growth Executive (Người lập kế hoạch / Owner).
  3. `Hoàng Minh Khôi` - Social Executive (Bài viết, content, social).
  4. `Nguyễn Ngọc Khánh` - Marketing Executive (Vận hành chiến dịch, paid ads).
  5. `Võ Thị Thu Hiền` - SEO Executive (Nội dung web, organic search).
  6. `Nguyễn Duy Quý` - Intern (Hỗ trợ thiết kế, assets).
...
| **2** | **Xác thực & Phân quyền giả lập** | Đổi user trong `UserSelector.jsx` chỉ đổi state phía client, không kiểm tra token/mật khẩu. Ai cũng có thể thao tác với vai trò của Manager. | Triển khai Authentication (NextAuth, Supabase Auth hoặc SSO Microsoft Entra ID của Mắt Bão) và áp dụng RBAC ở cấp API Backend. |
...
Dưới đây là tài liệu mô tả hiện trạng chi tiết của sản phẩm "Growth-loop Enterprise OS" - một hệ điều hành quản trị vận hành chiến dịch Growth Marketing cho doanh nghiệp Mắt Bão (gồm 2 khối: MBC - Hạ tầng Hosting/Domain và MBI - Hóa đơn điện tử/Hợp đồng số).
